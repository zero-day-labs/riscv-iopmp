...
module axi_to_detailed_mem #(
  ...
  always_comb begin
    ...
    if (r_cnt_q > '0) begin
      ...
      rd_meta        = rd_meta_d;
      rd_meta.addr   = rd_meta_q.addr;
      rd_valid       = 1'b1;
      if (rd_ready) begin
        r_cnt_d--;
        rd_meta_d.addr = rd_meta.addr + axi_pkg::num_bytes(rd_meta_q.size);
      end
    ...
    if (w_cnt_q > '0) begin
      ...
      if (axi_req_i.w_valid) begin
        ...
        if (wr_ready) begin
          ...
          w_cnt_d--;
          wr_meta_d.addr = wr_meta.addr + axi_pkg::num_bytes(wr_meta_q.size);
        end
      end
